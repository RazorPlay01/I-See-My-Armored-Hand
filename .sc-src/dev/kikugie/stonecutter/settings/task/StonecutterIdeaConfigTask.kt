...
public abstract class StonecutterIdeaConfigTask : DefaultTask() {
    ...
    @get:Input
    public abstract val projects: MapProperty<String, List<Identifier>>
    ...
    internal fun run() {
        val files = projects.get().entries.asSequence()
            .flatMap { (k, v) -> v.map { writeConfiguration(ProjectHierarchy(k), it) } }
            .toSet()
    ...
    private fun writeConfiguration(hierarchy: ProjectHierarchy, project: Identifier): String {
        ...
        val xml = TEMPLATE.getOrThrow()
            .replaceChecked("%FOLDER_NAME%", "Stonecutter${hierarchy.orEmpty()}")
            .replaceChecked("%ENTRY_NAME%", "Switch to $project")
            .replaceChecked("%TASK_NAME%", "${hierarchy.orEmpty()}:stonecutterSwitchTo$project")
...
private fun configurationFile(hierarchy: ProjectHierarchy, project: Identifier): String =
    "Stonecutter${hierarchy.orEmpty().formatPath()}_switchTo${project.formatPath()}.xml"

private fun String.formatPath(): String = replace(PATTERN, "_")
private fun String.replaceChecked(from: String, to: String): String =
